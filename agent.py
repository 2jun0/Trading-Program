...
class Agent:
	...
	# 매매 수수료 및 세금
	TRADING_CHARGE = 0 # 거래 수수료 미고려
	TRADING_TAX = 0

	# 행동
	ACTIONS = [Action.BUY, Action.SELL] # 인공 신경망에서 확률을 구할 행동들
	NUM_ACTIONS = len(ACTIONS) # 인공 신경망에서 고려할 출력값 개수

	def __init__(self, env: Env, min_trading_unit=1, max_trading_unit=2, delayed_reward_threshold=0.05):
		self.env = env
		...
		self.portfolio_value = 0 # balence + num_stocks * [현재 주식 가치]
		...
		self.ratio_portfolio_value = 0 # 포트 폴리오 가치 비율
	...
	def get_states(self):
		self.ratio_hold = self.num_stocks / int(
			self.portfolio_value / self.env.get_price())
	...
	def act(self, action, confidence):
		...
		if action == Action.BUY:
			...
			balance = self.balance - cur_price * (1 + self.TRADING_CHARGE) * trading_unit
			# 보유 현금이 모자랄 경우 보유 현금으로 가능한 만큼 최대한 매수
			if balance < 0:
				trading_unit = max(min(
					int(self.balance / ( cur_price * (1 + self.TRADING_CHARGE))), 
					self.max_trading_unit
				), self.min_trading_unit)
			# 수수료를 적용하여 총 매수 금액 산정
			invest_amount = cur_price * (1 + self.TRADING_CHARGE) * trading_unit
		...
		# 매도
			...
			trading_unit = min(
				self.decide_trading_unit(confidence), 
				self.num_stocks)
			# 세금과 수수료
			invest_amount = cur_price * (1 - (self.TRADING_TAX + self.TRADING_CHARGE)) * trading_unit
			self.num_stocks -= trading_unit # 보유 주식 수를 갱신
			self.balance += invest_amount # 보유 현금을 갱신
