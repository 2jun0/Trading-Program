...
class PolicyLearner:
	def __init__(self, stock_code, chart_data, training_data=None,
							...
							delayed_reward_threshold=0.5, lr=0.01):
		self.stock_code = stock_code # 종목코드
		self.chart_data = chart_data
		self.env = Env(chart_data) # 환경 객체
		# 에이전트 객체
		self.agent = Agent(self.env, min_trading_unit=min_trading_unit,
	...
	def fit(self, num_epoches=1000, max_memory_len=60, balance=100000, discount_factor=0, start_epsilon=.5, learning=True):
		logger.info(f"LR: {self.policy_network.lr}, DR: {discount_factor}, "
								f"TU: [{self.agent.min_trading_unit}, {self.agent.max_trading_unit}], "
								f"DRT: {self.agent.delayed_reward_threshold}")

		# 가시화 준비
		# 차트 데이터는 변하지 않으므로 미리 가시화
		self.visualizer.prepare(self.env.chart_data)
		...
		for epoch in range(num_epoches):
			...
			while True:
				...
				memory = [
					(memory_sample[i], memory_action[i], memory_reward[i]) for i in list(range(len(memory_action)))[-max_memory_len:]
				]
				if exploration:
					memory_exp_idx.append(itr_cnt)
					memory_prob.append([np.nan] * Agent.NUM_ACTIONS) #??#prob은 확률인듯. 이 경우에는 랜덤이니 nan
				else:
				...
				if delayed_reward == 0 and batch_size >= max_memory_len:
					delayed_reward = immediate_reward
				if learning and delayed_reward != 0:
					# 배치 학습 데이터 크기
					batch_size = min(batch_size, max_memory_len)
					# 배치 학습 데이터 생성
					x, y = self._get_batch(
			...
			self.visualizer.save(os.path.join(
				epoch_summary_dir, f'epoch_summary_{settings.timestr}_{epoch_str}.png'
			))
	...
	def _get_batch(self, memory, batch_size, discount_factor, delayed_reward):
		...
		for i, (sample, action, reward) in enumerate(
			...
			y[i, action] = (delayed_reward + 1) / 2 # y는 (지연보상 + 1)/2
			if discount_factor > 0 :
				y[i, action] *= discount_factor ** i
		return x, y

	def _build_sample(self):
		self.env.observe() # 다음 차트데이터의 행을 가져옴.
		if len(self.training_data) > self.training_data_idx + 1:
			self.training_data_idx += 1
			self.sample = self.training_data.iloc[self.training_data_idx].tolist() # 다음 학습데이터의 행을 가져옴 (리스트로)
			self.sample.extend(self.agent.get_states()) # 위 행의 agent의 상태를 추가
			return self.sample
		return None
