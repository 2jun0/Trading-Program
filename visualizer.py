...
class Visualizer:
	...
	def prepare(self, chart_data):
		...
		# 거래량 가시화
		x = np.arange(len(chart_data)) # x는 시간축
		volume = np.array(chart_data)[:, -1].tolist() # 거래량
		self.axes[0].bar(x, volume, color='b', alpha=0.3)
		# ohlc란 open, high, low, close의 약자로 이 속성으로 구성된 2차원 배열
		ax = self.axes[0].twinx()
		ohlc = np.hstack((x.reshape(-1, 1), np.array(chart_data)[:, 1:-1])) # 시가, 고가, 저가, 종가
	...
	def plot(self, epoch_str=None, num_epoches=None, epsilon=None,
				action_list=None, actions=None, num_stocks=None, outvals= None,
				exps=None, learning=None, initial_balance=None, pvs=None):
		# action_list는 [사기, 팔기]
		...
		for idx, outval in zip(x, outvals):
			color = 'white'
			if outval.argmax() == Action.BUY:
				color = 'r' # 매수면 빨간색
			elif outval.argmax() == Action.SELL:
				color = 'b'	# 매도면 파란색
	...
	def clear(self, xlim):
		...
		for ax in self.axes:
			...
			ax.get_yaxis().get_major_formatter().set_scientific(False)
			ax.ticklable_format(useOffset=False) # x축 간격을 일정하게 설정
